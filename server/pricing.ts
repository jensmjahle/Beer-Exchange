...
function raiseBoughtAndRebalance(
  ...
  const ctx = buildPricingContext(rows.map(r => ({
    id: r.id,
  ...
  while (Math.abs(excess) > 1e-6 && guard++ < 10) {
    if (excess > 0) {
      ...
      const totalSlack = others.reduce((a, r) => a + Math.max(0, r.current_price - r.min_price), 0)
      if (totalSlack <= 1e-9) {
      ...
      const others = rows.filter(r => r.id !== boughtId)
      const upSlack = others.reduce((a, r) => a + Math.max(0, r.max_price - r.current_price), 0)
      if (upSlack <= 1e-9) {
        const b = boughtId ? idToRow.get(boughtId) : null
        if (b) {
  ...
  for (const r of rows) {
    r.current_price = Math.max(r.min_price, Math.min(r.max_price, Number(r.current_price.toFixed(1))))
  }
  return rows
}

export async function recalcPricesForEvent(eventId: string, boughtBeerId: string | null, qty: number = 1) {
  // Load active beers
  let rows: Row[] = []
  if (db.kind === 'memory') {
  ...
  if (db.kind === 'memory') {
    ...
    const stmt = db.sql.prepare(`UPDATE event_beer SET current_price=? WHERE id=?`)
    ...
    try {
      ...
      for (const r of updated) {
        await client.query(`UPDATE event_beer SET current_price=$1 WHERE id=$2`, [r.current_price, r.id])
      }
