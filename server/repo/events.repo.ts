...
export async function listEvents(): Promise<Event[]> {
  ...
  if (db.kind === "sqlite")
    return db.sql
      .prepare(
        `SELECT * FROM event ORDER BY CASE status WHEN 'live' THEN 0 WHEN 'draft' THEN 1 ELSE 2 END, created_at DESC`,
      )
      .all();
...
export async function getEvent(id: string): Promise<Event | null> {
  if (db.kind === "memory")
    return db.mem.events.find((e) => e.id === id) ?? null;
  if (db.kind === "sqlite")
    return db.sql.prepare(`SELECT * FROM event WHERE id=?`).get(id) ?? null;
...
export async function createEvent(p: {
  ...
  if (db.kind === "memory") {
    db.mem.events.unshift(ev);
    return ev;
  }
  if (db.kind === "sqlite") {
    db.sql
      .prepare(
        `INSERT INTO event (id,name,currency,status,starts_at,ends_at,created_at,image_url) VALUES (?,?,?,?,?,?,?,?)`,
      )
      .run(
        ev.id,
        ev.name,
