...
export async function updateCustomer(
  ...
  if (db.kind === 'memory') {
    const i = db.mem.customers.findIndex(x => x.id === customerId && x.event_id === eventId)
    if (i < 0) return null
  ...
  if (db.kind === 'sqlite') {
    const c = db.sql.prepare(`SELECT * FROM customer WHERE id=? AND event_id=?`).get(customerId, eventId)
    if (!c) return null
    const next = {
    ...
    `).run(
      next.name, next.phone, next.shoe_size, next.weight, next.profile_image_url, next.work_relationship,
      next.gender, next.sexual_orientation, next.ethnicity, next.experience_level,
      customerId, eventId
    )
    return { ...c, ...next }
  }
...
export async function listCustomersWithStats(eventId: string) {
  if (db.kind === 'memory') {
    ...
    return base
      .map(c => ({ ...c, beers: agg.get(c.id)?.beers || 0, tab: agg.get(c.id)?.tab || 0 }))
