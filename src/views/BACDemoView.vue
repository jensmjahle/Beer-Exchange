...
<template>
  <div class="max-w-4xl mx-auto p-4">
    ...
    <div v-else class="space-y-6">
      ...
      <!-- BAC Display -->
      <div v-if="selectedCustomer" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <!-- Full BAC Display -->
        <div class="bg-bg2 p-4 rounded-lg">
        ...
        <div class="space-y-4">
          ...
          <div class="bg-bg2 p-4 rounded-lg">
            ...
            <div class="flex items-center gap-4 mb-3">
              <img v-if="selectedCustomer.profile_image_url"
              ...
              <div>
                ...
                <p class="text-sm opacity-70">{{ selectedCustomer.sexual_orientation }}</p>
              </div>
            </div>
            
            <div class="grid grid-cols-2 gap-2 text-sm">
              <div><strong>Weight:</strong> {{ selectedCustomer.weight }}kg</div>
              <div><strong>Gender:</strong> {{ selectedCustomer.gender }}</div>
              <div><strong>Phone:</strong> {{ selectedCustomer.phone || '—' }}</div>
              <div><strong>Work:</strong> {{ selectedCustomer.work_relationship || '—' }}</div>
            </div>
          </div>
          ...
          <div class="bg-bg2 p-4 rounded-lg">
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between p-2 bg-bg3 rounded">
                <span>Mini size:</span>
                <BACMini :customer="selectedCustomer" :event-id="eventId" size="mini" />
              </div>
              <div class="flex items-center justify-between p-2 bg-bg3 rounded">
                <span>Small size:</span>
                <BACMini :customer="selectedCustomer" :event-id="eventId" size="small" />
              </div>
              <div class="flex items-center justify-between p-2 bg-bg3 rounded">
                <span>Medium size:</span>
                <BACMini :customer="selectedCustomer" :event-id="eventId" size="medium" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="beers.length" class="bg-bg2 p-4 rounded-lg">
        <h2 class="text-lg font-bold mb-3">Available Beers ({{ beers.length }})*</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
          <div v-for="beer in beers.slice(0, 6)" :key="beer.id" class="bg-bg3 p-3 rounded">
        ...
        <p class="text-xs opacity-60 mt-3">
          *BAC calculations use actual ABV/volume data when available, 
          otherwise defaults to standard drink equivalents (12g alcohol).
        </p>
      </div>
      ...
      <div class="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
        ...
        <ul class="text-sm space-y-1 list-disc list-inside">
          ...
          <li>The BAC will be calculated based on their actual beer transactions</li>
          <li>BAC is shown in promille (‰) - Norwegian standard (0.5‰ = 0.05%)</li>
          <li>BAC updates automatically every minute to account for alcohol elimination</li>
          <li>Color coding: Green (safe), Yellow (caution), Orange/Red (impaired)</li>
          <li>Norwegian legal limit for driving is 0.2‰ (0.5‰ for serious offense)</li>
