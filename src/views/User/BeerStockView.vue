...
import { getBeerPriceHistory, getBeerStats } from '@/services/analytics.service.js'
  ...
  <section class="space-y-6">
    <header class="flex items-center justify-between">
      <div>
        ...
        <p class="opacity-70 text-sm">Event: {{ eventId }} · Beer: {{ stats?.name ?? stats?.beer_id ?? eventBeerId }}</p>
      </div>
      <button class="rounded-lg border px-3 py-1.5 border-[var(--color-border3)] hover:bg-[var(--color-bg4)]" @click="backToEvent">
        ← Back to Event
      </button>
    </header>
    ...
    <div class="flex items-center gap-2">
      <button
        v-for="r in ['1h','3h','day','all']" :key="r"
        class="rounded-lg px-3 py-1.5 border"
        :class="range === r ? 'bg-[var(--color-button1)] border-[var(--color-button1-border)]' : 'border-[var(--color-border3)] hover:bg-[var(--color-bg4)]'"
        @click="range = r"
      >{{ r === '1h' ? 'Last hour' : r === '3h' ? 'Last 3h' : r === 'day' ? 'Today' : 'All time' }}</button>
    </div>
    ...
    <span v-if="stats?.volume_ml" class="inline-flex items-center gap-1 px-2 py-1 rounded-full border border-border4 bg-bg4">
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        <div class="rounded-xl border p-4 bg-[var(--color-button4)]">
          ...
          <div class="font-bold">{{ stats?.cheapest?.customer_name ?? '—' }}</div>
          <div class="text-xs opacity-70" v-if="stats?.cheapest">paid {{ stats.cheapest.unit_price.toFixed(1) }} (×{{ stats.cheapest.qty }})</div>
        </div>
        <div class="rounded-xl border p-4 bg-[var(--color-button4)]">
          <div class="text-xs opacity-70">Most expensive buyer</div>
          <div class="font-bold">{{ stats?.priciest?.customer_name ?? '—' }}</div>
          <div class="text-xs opacity-70" v-if="stats?.priciest">paid {{ stats.priciest.unit_price.toFixed(1) }} (×{{ stats.priciest.qty }})</div>
        </div>
      </div>
    </div>
  </section>
