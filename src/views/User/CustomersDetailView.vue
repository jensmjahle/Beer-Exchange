...
<template>
  <section class="max-w-4xl mx-auto space-y-6 p-4">
    <header class="flex items-center justify-between">
      ...
      <button @click="router.back()" class="text-sm text-blue-600 hover:underline">← Back</button>
    </header>

    <div v-if="loading" class="p-4 border border-dashed rounded-xl">Loading…</div>
    <div v-else-if="error" class="p-4 border border-red-300 bg-red-50 text-red-700 rounded-xl">{{ error }}</div>
    ...
    <div v-else class="space-y-6">
      ...
      <div class="grid sm:grid-cols-3 gap-4">
        <div class="rounded-xl border p-4 bg-bg4 text-center">
          ...
          <div class="text-2xl font-bold">{{ details.summary?.beers ?? 0 }}</div>
        </div>
        <div class="rounded-xl border p-4 bg-bg4 text-center">
          <div class="text-sm opacity-70">Total spent</div>
          <div class="text-2xl font-bold">{{ details.summary?.tab?.toFixed(1) ?? '0.0' }} NOK</div>
        </div>
        <div class="rounded-xl border p-4 bg-bg4 text-center">
          <div class="text-sm opacity-70">Experience</div>
          <div class="text-2xl font-bold">{{ details.customer.experience_level || '-' }}</div>
        </div>
      </div>
      ...
      <div class="rounded-xl border p-4 bg-bg2">
        ...
        <ul class="divide-y">
          ...
          <li v-if="!details.transactions.length" class="italic opacity-60 py-2">
            No transactions yet
          </li>
        </ul>
      </div>
