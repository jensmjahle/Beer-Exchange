    ...
    <ul class="space-y-2">
      ...
      >
        <div
          ...
          :class="b.delta > 0 ? 'bg-green-500' : 'bg-red-500'"
        ></div>

        <div class="flex-1 min-w-0">
          <div class="font-semibold truncate">{{ b.name ?? b.beer_id }}</div>
          <div class="text-xs opacity-70">Now {{ fmt(b.current_price) }}</div>
        </div>
      ...
      <li v-if="!items.length" class="text-sm opacity-60 italic px-1">
        No movement yet
      </li>
    </ul>
  </div>
