...
const timeUntilSober = computed(() => {
  if (!bacData.value || bacData.value.bac === 0 || isNaN(bacData.value.bac)) return null
...
watch(() => [props.customer, props.eventId], () => {
  calculateBAC()
}, { deep: true })
...
<template>
  <div class="bac-display">
    ...
    <div v-else-if="error" class="p-3 bg-red-100 text-red-700 rounded-lg text-sm">
      ⚠️ BAC Error: {{ error }}
    </div>
    ...
    <div v-else-if="bacData" class="space-y-3">
      ...
      <div 
        ...
        :class="{
          'bg-green-50 border-green-200': bacStatus?.level === 'sober' || bacStatus?.level === 'minimal',
      ...
      >
        <div class="text-3xl font-bold mb-1" :style="{ color: bacStatus?.color }">
          {{ bacPercentage }}‰
        </div>
      ...
      <div class="grid grid-cols-2 gap-2 text-sm">
        ...
        <div class="bg-bg3 p-2 rounded">
          ...
          <div>{{ isNaN(bacData.hoursSinceFirstDrink) ? '0' : bacData.hoursSinceFirstDrink }}h</div>
        </div>
      </div>
      ...
      <div v-if="showDetails" class="mt-4 space-y-2">
        ...
        <div class="text-xs space-y-1 bg-bg3 p-3 rounded">
          <div><strong>Weight:</strong> {{ bacData.weightKg || 'Unknown' }} kg</div>
          <div><strong>Gender:</strong> {{ bacData.gender || 'Unknown' }}</div>
          <div><strong>Total alcohol:</strong> {{ isNaN(bacData.totalAlcoholGrams) ? '0' : bacData.totalAlcoholGrams }}g</div>
          <div><strong>Transactions:</strong> {{ bacData.transactions?.length || 0 }}</div>
        </div>
